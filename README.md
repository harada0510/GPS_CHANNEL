...
### 2_offset.py
変換された平面座標（絶対座標のまま）をスタート地点を軸にした相対座標系に変換（平面直交座標系Ver）<br>
スタート地点から各waypointに到着するまでの距離を```distance```として同出力ファイルに保存してる．<br>
ロボットのマップとして扱う場合はこの出力を用いる．
...
### 3_rotate.py
変換した２つの座標系を一緒にプロットするために，座標回転を行うプログラム．<br>
ロボットのマップには使わない．
